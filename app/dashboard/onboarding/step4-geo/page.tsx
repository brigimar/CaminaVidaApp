...
import { GeoForm } from './form';
import { useSaveGeoData } from './hooks';
import { useWizard } from '../wizard-context';
import { Loader2, ChevronLeft, AlertCircle } from 'lucide-react';

export default function Step4Geo() {
    const router = useRouter();
    const { updateStepData, nextStep, prevStep, state } = useWizard();
    // ID dinámico desde el contexto (Auth o Fallback UUID)
    const { loading, fetching, initialData, save, error } = useSaveGeoData(state.coordinatorId || '');

    const handleNext = async (data: any) => {
        const result = await save(data);
        if (result.success) {
            // Bridge: { provincias, localidades: {provincia, localidad}[] } -> Record<string, string[]>
            const localidadesRecord: Record<string, string[]> = {};
            data.localidades.forEach((l: any) => {
                if (!localidadesRecord[l.provincia]) localidadesRecord[l.provincia] = [];
                localidadesRecord[l.provincia].push(l.localidad);
            });

            updateStepData(4, {
                provincias: data.provincias,
                localidades: localidadesRecord
            });
            nextStep();
        }
    };
    ...
    const formDefaults = state.geoData.provincias.length > 0
        ? {
            provincias: state.geoData.provincias,
            localidades: Object.entries(state.geoData.localidades).flatMap(([prov, locs]) =>
                (locs as string[]).map(loc => ({ provincia: prov, localidad: loc }))
            )
        }
        : initialData;
            ...
            <div className="relative z-10 w-full max-w-xl">
                <header className="mb-16 space-y-4">
                    ...
                    <h1 className="text-5xl font-black text-slate-900 leading-tight">
                        ¿Dónde te gustaría <span className="text-green-600 italic">caminar?</span>
                    </h1>
                    {error && (
